    ...
    <section>
      <div className="bg-purple-dark">
        ...
        <div className="px-8 py-14 text-brand-white text-center max-w-[1140px] mx-auto">
          <span className="inline-block bg-brand-white/10 text-white text-xs font-semibold tracking-widest uppercase px-4 py-1.5 rounded-sm mb-4">
            Impact of Donating
          </span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 sm:grid-cols-1 gap-6">
            {impactStats.map((s) => (
              <div key={s.label} className="bg-brand-white rounded-sm py-8 px-4">
                <p className="text-4xl font-extrabold text-purple-dark mb-2">{s.value}</p>
                <p className="text-sm text-brand-black leading-snug">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
