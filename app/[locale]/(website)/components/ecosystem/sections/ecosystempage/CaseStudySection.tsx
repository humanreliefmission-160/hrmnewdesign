...
export default function CaseStudySection({ stage }: Props) {
  ...
  const imageUrl =
    caseStudy.image?.asset
      ? urlFor(caseStudy.image.asset).width(800).height(600).fit("crop").auto("format").quality(80).url()
      : null;

  return (
    <section className="bg-brand-white/50 py-8 sm:py-16 px-6 md:px-12 lg:px-24 shadow-lg my-12 max-w-285 mx-3 sm:mx-auto rounded-sm">
